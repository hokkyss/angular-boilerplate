 ...
 * ```
 */
export type KeyIn<Key, Type extends object> = Key extends keyof Type
  ? true
  : false;
...
export type SnakeToCamelCase<Key extends string> =
  Key extends `${infer First}_${infer Second}`
 ...
 * ```
 */
export type CamelToSnakeCase<Key extends string> =
 ...
 * ```
 */
export type InclusiveRange<F extends number, T extends number> =
  | Range<F, T>
  | T;
...
type Iterator<
  ...
  N extends number,
  Iter extends T[] = [],
> = Iter['length'] extends N ? Iter : Iterator<T, N, [...Iter, T]>;
 ...
 * ```
 */
export type Member<T> = T extends Array<infer U>
  ? U
